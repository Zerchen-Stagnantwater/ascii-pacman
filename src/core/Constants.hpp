...
inline constexpr float POWER_WARNING_TIME = 2.f;
inline constexpr float PACMAN_MAX_SPEED = 0.08f;
inline constexpr float GHOST_MAX_SPEED = 0.10f;
inline constexpr float SPEED_SCALE = 0.01f; // subtracted per level
inline constexpr int DOT_POINTS = 10;
inline constexpr int PELLET_POINTS = 50;
inline constexpr int GHOST_POINTS = 200;
inline constexpr int GHOST_COMBO_BASE = 200;
inline constexpr int WINDOW_W = COLS * CELL_SIZE;
inline constexpr int WINDOW_H = ROWS * CELL_SIZE + 40;
inline constexpr const char *SAVE_PATH = "save.json";
} // namespace Config
